"""Scanned PDF parser: layout detection + OCR → PDFDocument.

Renders each PDF page to a numpy image, runs a layout detector to segment the
page into typed regions, then runs OCR on every region concurrently within
each page.  Pages are processed sequentially.  The output ``PDFDocument`` uses
the same ``ContentBlock``-based schema as ``DigitalPDFParser``, so the
existing formatter and extractor work unchanged.
"""
