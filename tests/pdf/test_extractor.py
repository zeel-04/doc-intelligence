...
from tests.conftest import FakeFormatter, FakeLLM, SimpleExtraction
...
class TestExtractSinglePassNoCitations:
    def test_returns_extracted_data_and_metadata(
        ...
        result = extractor.extract(
            document=doc, request=request, formatter=FakeFormatter()
        )
    ...
    def test_llm_config_forwarded(self, extractor_with_llm, sample_pdf: PDF):
        ...
        extractor.extract(document=doc, request=request, formatter=FakeFormatter())
...
class TestExtractMultiPass:
    ...
    def _make_doc(self, sample_pdf: PDF) -> PDFDocument:
        return PDFDocument(uri="test.pdf", content=sample_pdf)

    def _make_request(self, citations: bool = True) -> PDFExtractionRequest:
    ...
    def test_multi_pass_no_citations_returns_pass1_result(self, sample_pdf: PDF):
        ...
        result = extractor.extract(
            document=self._make_doc(sample_pdf),
            request=self._make_request(citations=False),
    ...
    def test_multi_pass_with_citations_calls_llm_three_times(self, sample_pdf: PDF):
        ...
        extractor.extract(
            document=self._make_doc(sample_pdf),
            request=self._make_request(citations=True),
            formatter=FakeFormatter(),
        )
    ...
    def test_multi_pass_invalid_mode_raises(self, sample_pdf: PDF):
        ...
        extractor = PDFExtractor(llm=llm)
        doc = PDFDocument(uri="test.pdf", content=sample_pdf)
        request = PDFExtractionRequest(uri="test.pdf", response_format=SampleResponse)
        request.extraction_mode = MagicMock()  # not a real PDFExtractionMode
...
class TestExtractPageNumbers:
    """page_numbers on PDFExtractionRequest is forwarded to the formatter in all modes."""

    _BBOX = BoundingBox(x0=0.0, top=0.0, x1=1.0, bottom=0.1)

    def _make_multipage_doc(self) -> PDFDocument:
        ...
        return PDFDocument(uri="test.pdf", content=PDF(pages=pages))
    ...
    def test_single_pass_only_sends_requested_pages_to_llm(self):
        ...
        request = self._make_request(page_numbers=[1])

        extractor.extract(document=doc, request=request, formatter=PDFFormatter())
    ...
    def test_single_pass_none_page_numbers_sends_all_pages(self):
        ...
        extractor = PDFExtractor(llm=llm)
        doc = self._make_multipage_doc()
        request = self._make_request(page_numbers=None)
    ...
    def test_single_pass_multiple_page_numbers_filters_correctly(self):
        ...
        extractor = PDFExtractor(llm=llm)
        doc = self._make_multipage_doc()
        request = self._make_request(page_numbers=[0, 2])
